"""Leakage gate for the goals/assists feature builder (mirrors minutes test).

Features for season N must be byte-identical whether built over
[N-2, N] or [N-2, N+M]. Adding later seasons must not change earlier-season
feature values.
"""
...
@pytest.mark.leakage
def test_goals_features_pit_stable_across_truncation() -> None:
    ...
    # Check the most leak-prone columns: rolling features and labels
    leak_cols = [c for c in FEATURE_COLUMNS if c.startswith(("xg_", "npxg_", "xa_", "shots_", "key_passes_"))]
    leak_cols += ["goals", "assists"]
